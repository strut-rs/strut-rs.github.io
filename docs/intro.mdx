...
Strut is a [Rust library crate][crates-io-strut].
...
Strut has many features, but none of them are enabled by default.
...
- [`dotenvy`][crates-io-dotenvy]: to support environment defaults in `.env` files.
- [`parking_lot`][crates-io-parking-lot]: for performant locks.
...
:::info[no-`std`?]
Strut is not designed for no-`std` applications (e.g., embedded software).
